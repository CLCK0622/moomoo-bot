...
## 5a. DSR 的 V-family —— 如实披露（工部 e71201ec 2026-07-30 查出，据实记录，不粉饰）

决定 DSR 里 V（试验 Sharpe 方差）的那组陡度 family = **`{(0.50,0.00) 主, (0.75,0.25), (1.00,0.00)}`**。
**诚实交代**：本组 family 是在 `run_carry_rates.py` 代码里写定的、**属"跑时选定"，v2(`5d25064`) 冻结的预注册
正文并未逐格列出这三格**（§5 当时只固定了主格 (0.50,0.00)）。工部实测指出：V 对 family 极敏感
（as-run V=8.65e-6→DSR 0.926；V×4→0.843；V×9→0.714；V×25→0.377），故"事后挑紧 family"是一个能自行
调松 haircut 的旋钮（同 N/V/成本/ADV/台账/ppy 之后的第七种 fail-open 形态）。
- **本轮不受影响**：A 的 standalone 负向**不依赖 DSR 那条 0.926 擦边**——CAGR 4.30%/MDD 27.10% 在官方/影子/
  兜底三线上两轴皆不过，V 就算翻大照样判负。
- **对 B/C 的硬约束**：family（决定 V 的那组试验）今后必须像 universe/阈值一样**预注册冻结、跑后不得增删**；
  且登记台账时每条须带 `trial_sharpes`，否则 pooled-V 地板退化（工部 e71201ec §3 更正）。本文件 v3(`16be273`)
  起把上述三格如实列入，标注为「v2 跑时选定、非冻结前」，供都察院核。

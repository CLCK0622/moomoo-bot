...
**冻结于任何回测结果被读取之前。** 基线 rebase 到 `agent/evo-162-residual-reversal @ ec0aeab`（DSR 单位
修正含反方向洞已闭合、77 单测绿、canonical 台账 N=29）。**信号从零写，不接 VIX carry（EVO-25 已证伪）那套。**
本文件在结果 commit 之前提交，git 时序供户部/都察院核。

> 硬前置**已闭合**（工部 2026-07-29，`ec0aeab`）：DSR 产出侧去年化 + 门侧归一/软旗 + 反方向 ppy 洞全落地。
> 数据齐（fred_yields.parquet + BIL/IEF/TLT 含息复权）。故本候选**本轮可出正式 verdict**。
...
- 新代码：`swing/carry_rates_signals.py`（信号，从零）+ `run_carry_rates.py`（接线）。判定 100% 交
  `research/gate.certify()`（`ec0aeab` 版，带 N/V/成本/容量/ppy 修复）。复用 `carry_evaluate`/`run_carry`
...
- rebase `ec0aeab`；`project_ledger()` 读入库 `trial_ledger.jsonl`（N=29），`register_run` 后连 `.jsonl` 提交。
...
- **DSR trial Sharpe 按每期口径**（`r.mean()/r.std()`，无 `*sqrt(252)`）**且不声明 `trials_periods_per_year`**
  ——工部 `ec0aeab` 新加的反方向软旗会对「已是每期却又报 ppy」打「过松」旗，故绝不传 ppy。

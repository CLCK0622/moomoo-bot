...
def main(argv=None) -> int:
    ...
    # 注意：export 内部的 register_run 不支持 candidate_id/supersedes，故此处**不传 ledger**，
    # 由本 runner 在下方显式登记（工部 2026-08-06 裁定：N 走 (b) supersedes 覆盖、维持 47）。
    man = factor_export.export(Path(args.store), Path(args.factors_out), start=args.start, end=args.end,
                               factor_set="mining_i", repo_root=_REPO_ROOT)
    ...
    # 3) 台账：N 走 (b) —— candidate_id + supersedes 覆盖 run1，同一批 12 条试验只计一次（维持 47）
    n_before = ledger.cumulative_n()
    rec = ledger.register_run(run_id=f"alpha_mining_i-{args.prereg_commit}", source="qlib",
                              n_trials_total=man.get("n_expressions_attempted", len(MINING_FACTORS)),
                              n_evaluated=man.get("n_exported", len(MINING_FACTORS)),
                              trial_sharpes=trial_sharpes,
                              candidate_id="alpha_mining_i", supersedes=args.supersedes,
                              note="(i) 有界因子挖掘 run2：方向移入 MINING_DIRECTION（Qlib 不支持算子前置负号），"
                                   "因子定义与冻结 §2 逐条一致；supersedes 覆盖 run1，同批试验只计一次")
    n_cum = ledger.cumulative_n()
    print(f"[ledger] before={n_before} after={n_cum} (supersedes={args.supersedes!r}) run_id={rec.run_id}")
    ...
    report = {"issue": "EVO-8", "candidate": "alpha_mining_i", "preregistration_commit": args.prereg_commit,
              ...
              # —— 工部 2026-08-06 裁定的三项留痕 ——
              "ledger_ruling_b_supersedes": {
                  "rule": "N 走 (b)：同一批 12 条因子定义只计一次试验",
                  "supersedes": args.supersedes, "candidate_id": "alpha_mining_i",
                  "cumulative_n_before": n_before, "cumulative_n_after": n_cum,
                  "expected": 47, "ok": bool(n_cum == 47)},
              "oos_ruling_a_second_shot": {
                  "rule": "OOS 走 (a)：如实记为同候选第 2 发（run1 已看过一次样本外，不因无效而当没看过）",
                  "run1": {"status": "工具缺陷导致的无效执行（Qlib 解析器静默丢弃 7/12 因子）",
                           "oos_key": "319d80a89aabefaa573f51385b6d45e340c286185d6fcaf26caa9b34db669db6",
                           "consumed": 1, "result_5factor": "REJECTED_dsr, CAGR 11.55% / MDD 37.91%"},
                  "run2": {"status": "本次＝第 2 发（冻结 12 因子齐全口径）",
                           "note": "新 frozen_hash 会另发一票，故在此显式披露、不让其静默通过"}},
              "direction_refactor_equivalence": {
                  "why": "Qlib 表达式解析器不支持算子前置一元负号（'-Mean(...)' → TypeError）",
                  "what": "方向从表达式移到 MINING_DIRECTION 映射；因子定义/经济含义与冻结 §2 逐条一致，无一条被重定义",
                  "table": [{"factor": k, "prereg_expr_v1": ("-" + v if MINING_DIRECTION[k] < 0 else v),
                             "run2_expr": v, "run2_direction": MINING_DIRECTION[k],
                             "equivalent": True} for k, v in MINING_FACTORS.items()],
                  "superseded_frozen_hash_kept": "run1 的 frozen_hash 记为 superseded、保留不删（见 run1_defect_disclosure）"},

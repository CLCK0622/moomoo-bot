...
**周度盯市上界（冻结 `rebalance: weekly` 的直接实现）：**第 N 轮只在 `[entry_N, entry_{N+1})` 盯市；
最后一轮开口至最新归档 bar。`entry_{N+1}` 必须由归档中的真实 bar 观察得到，不能用假日表预测。没有此上界
会把每轮误算成持有到今天、重复计入行情，并使扫描需求集随历史轮次发散。验收层仍只用 OpenD qfq 收益率，
带本标注的下界读数永不与验收数并列上报。

...
**首份归档前两轮的受限回补（吏部 2026-08-31）：**下界层的准入是「该 bar 已存在于不可改、带内容
哈希、append-only 的归档」，**不再按 bar 日期设 floor**；这不是把事后价格说成当时观察。凡使用日期早于
首份归档的 bar，整段 NAV（逐点、序列与汇总）均带独立 `bar_provenance`：它与 `reading_kind` 正交，明确
「无当时副本、逐位比护栏无基线、不得称已核验或已交叉核对、不得升格为验收数」。

**一次性回补授权，覆盖「在归档 + 扫描机制完整落地之前已落盘的全部轮次」**——现为 08-10、08-31；若
09-07 及其后若干轮仍早于该机制上线，同样纳入。
**机制到位之后**：某个 `(symbol,date)` 扫描**有机会归档而没有归档**、且已滑出 compact 窗口 ⇒ 那一轮的
读数就是丢了，**停下回报，不得回补、不得援引本裁定**。
判据是「机制存在且有过时间窗口去工作」，不是某个日期。

扫描腿只在非轮次日（周二至周五）运行，走 MARKING、同一供应商同一端点 AV `TIME_SERIES_DAILY compact`，
按**全部已落盘轮次**重建仍缺的键并幂等追赶；漏跑一次，下次必须补齐，而非只看最近一轮。它只请求仍缺键
对应标的与当前开窗持仓，绝不刷新 `daily_full`，所有返回仍按同一 schema append-only 归档；差异继续走
RESOLUTION，绝不原地改写。扫描产物必须列出仍缺 `(symbol,date)` 数、最老缺口距 compact 窗口滑出的交易日
余量；任一缺口余量 ≤20 个交易日即落硬 ALERT、破静默回报吏部（仅供人工处理，不进入统计）。

**周度盯市上界与连乘（冻结 `rebalance: weekly` 的直接实现）：**每个格的第 N 段只在
`[entry_N, 该格下一次真正建立新 book 的 entry)` 盯市；最后一段开口至最新归档 bar。上界必须由归档中的
真实 bar 观察得到，不能用假日表预测；某格未调仓不是新 book，不能截断其他格，也不能把该格伪造成现金平线。
...
须归档上一段旧持仓在这个上界日的 open，尽管该日不属于上一段 NAV marks。没有这些规则会把每轮误算成持有到
今天、重复计入行情，并使扫描需求集随历史轮次发散。验收层仍只用 OpenD qfq 收益率，带本标注的下界读数永不
与验收数并列上报。

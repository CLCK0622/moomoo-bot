...
| 结算口径 | **两腿同口径、都取总回报**（§8.4）；权益腿总回报源**当前不可用**，数据侧不开工，等吏部给字（§8.5） |
...
| 批次 | 内容 | 期限 |
|--|--|--|
| 1 | bar 归档（append-only + 内容哈希 + 取数时点）、派生结算层（**含权益腿总回报口径**）、第 1 轮回补结算、**(b) 接管** | **2026-10-31** |
...
**过渡期口径（批次 1 → 批次 3 之间）必须在报告里写明**：那段时间是「**权益腿总回报 + 现金腿零收益**」。
该组合让全现金格相对在场格**偏低** ⇒ 对「违规不调仓」朝严，可接受；**但它不是终值口径**，
中间读数不得当终值报。
...
**受阻范围要说准**（这一条我与工部尚书一致，并补上理由）：

| 批次 1 交付 | 是否受阻 | 理由 |
|--|--|--|
| 接口设计（§8.4 契约） | 否 | 契约与数据源无关 |
| bar 归档 | 否 | 归档的是轮内 AV as-traded bars，本来就取得到 |
| **(b) 接管判据** | **否** | 见下 |
| **第 1 轮回补结算** | **是** | 需要持有期的总回报序列，当前零覆盖 |

**(b) 接管判据为什么不受阻**——派生层有两个用途，只有一个需要总回报：

* **执行器等价性**（接管判据）：把**同一个**派生函数分别作用于 (a)/(b) 两侧的 round JSON，比两边输出
  是否逐位相同。这里只要求两侧**用同一个价格基准**，用什么基准不影响结论；归档的 as-traded bars 足够。
* **验收读数**：需要总回报，受阻。

⇒ 接管可以在总回报源到位之前就验完。**但必须配一条硬性标注**：等价性用的那份输出**不是验收数**，
产物里要显式标死，绝不能被读成 verdict——否则就成了拿 as-traded 读数当业绩，正是 §8.4 禁的事。

**处置**：数据侧不开工，等吏部对「OpenD 可用性作为批次 1 前置依赖上报，还是改口径」给字。
本文件只记事实与契约，不自行放宽、不自行升报。

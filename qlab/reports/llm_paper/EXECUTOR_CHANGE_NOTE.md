...
**一次性授权，且自动关闭**——这是防先例被拉长的那道闸：本授权**只适用于首份归档之前已落盘的轮次**
（08-10、08-31）。**此后任何一轮若没归档到自己的 bar，那就是 bug，那一轮的读数就是丢了——停下回报，
不得回补、不得援引本裁定。**优先用例行取数已归档的同一供应商同一端点 AV 数据；确实未覆盖才允许一次
定向补取，走 MARKING 配额、如实记账、按同一 schema 归档，且必须在非轮次日执行（16 个既有标的与周一
轮次约 9 次调用合计正好触及免费 25/日上限）。验收层仍只用 OpenD qfq 收益率，带本标注的下界读数永不
与验收数并列上报。
